Ext.define('Siace.controller.dataGeneral.WindowFileUpload', {
	extend: 'Ext.app.Controller',
	views: [
		'dataGeneral.WindowFileUpload'
	],

	init: function(application) {
		this.control({
			'windowfileupload': {
				//beforerender: this.pub_BeforeRender,
			},
	...
	wfu_btnAttachClick: function(button, e, options) {
			...
			Ext.Msg.confirm(translations.confirmar, translations.mensaje_pregunta_guardar, function(btn){
				if ( btn === 'yes' ) {
			        ...
			        _form.getForm().submit({
						...
						params:{ xx0005: 'OK', 
						         xxSchema_table: _win.getSchemaTable(), xxTable_field: _win.getTableField(), xxRecord_key: _win.getRecordKey(),
						         xxRecord_field: _win.getRecordField(), xxTable: _win.getTable(), xxField: _win.getField(),
		...
		//var _win = button.up('window'); button.up('window').destroy();
	},

	wfu_btnFileDownloadClick: function(button, e, options) {
		var _file = button.up('window').down('#ffiFile').fileInputEl.dom.files[0];
		fext_FileDownload(_file, undefined);
	},


	wfu_ffiFileChange: function(filefield, value, options) {
		var _win = filefield.up('window');
		var _r = fext_FileReader(filefield, _win.getTypeFile(), _win.getTypeMessage(), _win.getSizeMax(), _win.getSizeMessage(), '', _win.down('#btnFileDownload'));
